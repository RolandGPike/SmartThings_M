 ...
 *	Author:				Chris Charles, edited by Marco Raimondi
 *
...
metadata {
	definition (name: "Fibaro UBS", namespace: "cjcharles0 + MR", author: "Chris Charles + MR") {
...
tiles(scale: 2) {
	...
	valueTile("configure", "device.configure", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
		state "default", label:'Send Config', action:"updateCurrentParams"//, icon:"st.secondary.configure"
	}
	valueTile("report", "device.report", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
		state "default", label:'List Config', action:"listCurrentParams"
	}
    standardTile("refresh", "device.refresh", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
        state "default", action:"refresh.refresh", icon:"st.secondary.refresh"
    }
    valueTile("createchildren", "createchildren", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
		state "default", label:'Create Contact Children', action:"createChildDevices"
	}
    valueTile("createtempchildren", "createtempchildren", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
		state "default", label:'Create Temp. Children', action:"createChildTempDevices"
	}
    valueTile("removechildren", "removechildren", inactiveLabel: false, decoration: "flat", width: 2, height: 1) {
...
details(["contact1","contact2",
        ...
        "configure", "report","refresh" ,"createchildren", "createtempchildren", "removechildren"])
...
preferences {
		...
		input name: "param14", type: "number", range: "0..1", required: true, //defaultValue: "0",
            title: "Parameter No. 14 - Scene activation functionality." +
                   ...
                   "Default value: 0."
		input name: "autoUpdate", type: "boolean", required: true, defaultValue: false,
            title: "Parameter Autoupdate - Parameter that activates the updating of the temperature and status of the contacts.\n" +
                   "Default value: false."
		input name: "autoUpdate_timer", type: "number", range: "30..3600", required: true, defaultValue: "1800",
            title: "Parameter Autoupdate Time - This parameter is used to define the update time. Valid if Autoupdate parameter is set to on.\n" +
                   "Available settings:\n" +
                   "30-3600 - Seconds\n" +
                   "Default value: 1800."                  
	}
...
def updated() {
    ...
    if(autoUpdate == "true") 
    {
        log.debug "Timer scheduled every ${autoUpdate_timer.value} seconds"
    	runIn(autoUpdate_timer.value, "refresh") 
    }
...
def refresh() {
    ...
    def cmds = []
    log.debug "Refresh"
	cmds << response(encap(zwave.basicV1.basicGet()))
...
//Update status when receive basic report
